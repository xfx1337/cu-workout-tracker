...
push в mattermost
      ...
      └─ сборка образа  →  registry.gitlab/<группа>/<проект>/bot:<sha>
                │
                ├─ тесты:домен       ← блокируют выкладку
                ├─ тесты:диспетчер   ←
                ├─ тесты:postgres      не блокируют
                ├─ тесты:живые         не блокируют
                │
...
**Сначала образ, потом тесты внутри него.** В закрытом контуре интернет есть не
у каждого раннера, а образ собирать всё равно надо — пусть сборка и будет
единственным местом, которому нужны pypi и apt. Тестам сеть уже не нужна.
...
прислать клик, а он проверяется тремя способами (см. раздел 7).
...
| переменная | зачем |
|---|---|
| `PIP_INDEX_URL`, `PIP_TRUSTED_HOST` | внутреннее зеркало pypi, если наружу закрыто. pip читает их сам |
| `MM_URL`, `MM_TOKEN` | включают живые тесты против TiMe. Без них набор просто не запускается |

**Секреты бота в переменные CI не кладём.** `MM_TOKEN` для рантайма и пароль базы
живут в `.env` на сервере: у CI нет причин их знать, а чем меньше мест с секретом,
тем лучше. Registry-логин GitLab подставляет сам через `CI_REGISTRY_*`.

### Корпоративные сертификаты

Если в сети стоит MITM-прокси, положи его корневые `.crt` в каталог `certs/`
рядом с `Dockerfile` — сборка подмешает их к системным. Файлы в git не попадают.
Если прокси нет, каталог остаётся пустым и образ собирается как обычно.

Пользователь `deploy` на сервере должен состоять в группе `docker`.

---

## 5. Какие тесты блокируют выкладку

Выкладка ждёт только два набора — **домен** и **диспетчер**. Они не ходят в сеть
вообще и обязаны проходить в любом окружении: если они красные, значит сломан код.

Остальные помечены `allow_failure` и выкладку не останавливают:

| набор | почему не блокирует |
|---|---|
| `тесты:postgres` | нужен раннер с поддержкой `services`; не умеет — это особенность раннера, а не поломка кода |
| `тесты:живые` | требуют доступного Mattermost; на закрытом контуре его может не быть |

Живые тесты вдобавок **сами определяют, что стенда нет**: недоступный сервер,
неверный токен или кривой URL дают пропуск с понятным сообщением и успешным
завершением. Падать они будут только на реальной ошибке в нашем коде.
Включаются, лишь если в переменных CI заданы `MM_URL` и `MM_TOKEN`.

Так что первый запуск не заблокируется из-за того, что TiMe ещё не подключён.

## 6. Как выкладывать

Push в `mattermost` собирает образ и гоняет тесты. Выкладка — **вручную**:
в пайплайне у задачи `выкладка` нажимается кнопка. Так релиз не случается сам
по себе от любого коммита.
